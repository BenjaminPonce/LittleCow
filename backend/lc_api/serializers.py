...
class GrupoSerializer(serializers.ModelSerializer):
    class Meta:
        model = Grupo
        fields = ['id', 'nombre']  # Solo campos que existen
    
    def to_representation(self, instance):
        representation = super().to_representation(instance)
        try:
            jefe = instance.jefe.first()
            if jefe:
                representation['jefe'] = {
                    'id': jefe.id,
                    'usuario': jefe.usuario.username
                }
            else:
                representation['jefe'] = None
        except Exception:
            representation['jefe'] = None
        return representation
    ...
    class Meta:
        ...
        extra_kwargs = {
            'porcentaje': {
                'min_value': 0, 
                'max_value': 100,
                'decimal_places': 2  # Añade precisión decimal
            },
            'ingreso_personal': {
                'min_value': 0,
                'decimal_places': 2  # Añade precisión decimal
...
class GastoCompartidoSerializer(serializers.ModelSerializer):
    class Meta:
        ...
        fields = ['id', 'monto_total', 'distribucion', 'grupo']
        extra_kwargs = {
